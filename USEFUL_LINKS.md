# 🔗 Useful Resources & Tools

A curated list of helpful websites, tools, and documentation for Data Science and Python development.

## 🛠️ Tools & IDEs

| Resource | Description | Category | Link |
|:--- |:--- |:---:|:---:|
| **Thonny IDE** | A beginner-friendly Python IDE with built-in debugger and variable visualization. | 🖥️ IDE | [Visit Site](https://thonny.org/) |

---

## 📚 Documentation & Libraries

| Resource | Description | Category | Link |
|:--- |:--- |:---:|:---:|
| **Python 3 Docs** | Official Python documentation and tutorials. | 📘 Docs | [Visit Site](https://docs.python.org/3/) |
| **NumPy** | The fundamental package for scientific computing with Python. | 🧮 Lib | [Visit Site](https://numpy.org/) |
| **Pandas** | Fast, powerful, flexible and easy to use open source data analysis and manipulation tool. | 🐼 Lib | [Visit Site](https://pandas.pydata.org/) |

---

## 🎓 Courses & Tutorials

| Resource | Description | Category | Link |
|:--- |:--- |:---:|:---:|
| **Math for Deep Learning** | Comprehensive course on mathematical foundations for Deep Learning. | 📺 Course | [Watch](https://www.youtube.com/watch?v=kyQ0CRkYhy4&list=PLUl4u3cNGP60YyhMjYmXuVmX562QcClSp) |
| **Deep Learning Series** | In-depth tutorials covering Deep Learning concepts and implementation. | 📺 Course | [Watch](https://www.youtube.com/watch?v=DrhJLHiia7g&list=PLgPbN3w-ia_PeT1_c5jiLW3RJdR7853b9) |
| **Deep Learning Course** | Extensive video series covering Deep Learning fundamentals and applications. | 📺 Course | [Watch](https://www.youtube.com/watch?v=1nqCZqDYPp0&list=PLTKMiZHVd_2KJtIXOW0zFhFfBaJJilH51) |
| **DataCamp** | Interactive learning platform for Data Science and AI. | 🎓 Platform | [Visit Site](https://app.datacamp.com/) |
| **MIT OCW (EECS)** | Free lecture videos and materials for Electrical Engineering & Computer Science. | 🎓 Course | [Visit Site](https://ocw.mit.edu/search/?d=Electrical%20Engineering%20and%20Computer%20Science&f=Lecture%20Videos&f=Lecture%20Audio&s=department_course_numbers.sort_coursenum) |
| **fast.ai** | Practical Deep Learning courses focused on coding and implementation. | 🎓 Course | [Visit Site](https://www.fast.ai/#category=courses&listing-listing-page=2) |

---

## 🗺️ Roadmaps & Career

| Resource | Description | Category | Link |
|:--- |:--- |:---:|:---:|
| **AI Data Scientist Roadmap** | A step-by-step guide to becoming an AI Data Scientist. | 🗺️ Roadmap | [Visit Site](https://roadmap.sh/ai-data-scientist) |

---

*Feel free to add more useful links to this list!*
